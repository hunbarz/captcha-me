...
def chat_internvl_api(question = "Where is D.B. Cooper?", image = ""):
    characters = string.ascii_letters + string.digits
    sessionid = ''.join(random.choice(characters) for _ in range(11))

    # Create a session object
    rs = requests.Session()
    rs.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'})
    ...
    # 1. Upload the image
    response = rs.post(f'https://opengvlab-internvl.hf.space/upload?upload_id={sessionid}', files=files)
    
    fp   = json.loads(response.text)
    data = json.dumps({
        "data": [
            None, 
            {
                ...
                {
                    "path": f"{fp[0]}",
                    "url": f"https://opengvlab-internvl.hf.space/file={fp[0]}",
                    "orig_name": f"{fp[0].split('/')[-1]}",
                    "size": size,
                    "mime_type": "image/png",
        ...
        "event_data": None, "fn_index": 10, "trigger_id": 28, "session_hash": f"{sessionid}"
    }, ensure_ascii = False) # make sure 

    # 2. Push your image/question in the queue
    response = rs.post(f'https://opengvlab-internvl.hf.space/queue/join', data=data)

    # 3. Retrieve queue 1 
    response = rs.get(f'https://opengvlab-internvl.hf.space/queue/data?session_hash={sessionid}')
    ...
    data = {
        ...
        "fn_index": 11,
        "trigger_id": 28,
        "session_hash": f"{sessionid}"
    }

    # 4. Push again to the queue, different values 
    response = rs.post(f'https://opengvlab-internvl.hf.space/queue/join?', json=data)

    # 5. Retrieve answer to your question
    response = rs.get(f'https://opengvlab-internvl.hf.space/queue/data?session_hash={sessionid}')
    
    answer = re.search('data: {"msg":"process_completed","event_id":".*","(.+)"]],{"interactive"', response.text, re.MULTILINE)
    return answer.group(1).strip().lower()
...
def solve_captcha(captcha_path):
    ...
    instruction = chat_internvl_api("What object in the squares or images should be selected according to the text in the blue box at the top of the image? Answer with maximum 2 words", solved_captcha_path)
    ...
    for i, img in enumerate(subimages_captcha):
        res = chat_internvl_api(f"Does the image contain {instruction}, or parts of it? Answer with a simple YES or NO.", img)
...
solve_captcha("./captchas-internvl-api/image-02.png")

#for captcha_path in sorted(list_directory_contents("./captchas-internvl-api")):
#    if "solved" not in captcha_path or "solving" not in captcha_path:
#        print(f"\n[+] Solving CAPTCHA -> {captcha_path}")
#        solve_captcha(captcha_path)

# $ time python3 solve-captcha-internvl-api.py
# single image --> 51s
# 25 images -----> 21m41s 